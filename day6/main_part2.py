...
def parse_lines(lines: list[str]) -> RaceInfo:
    times = lines[0]
    distances = lines[1]
    times = times.split(":")[1].strip()
    distances = distances.split(":")[1].strip()
    time = "".join([x for x in times.split(" ") if x != ""])
    distance = "".join([x for x in distances.split(" ") if x != ""])
    return RaceInfo(time=int(time), distance=int(distance))
